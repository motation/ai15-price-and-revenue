...
//                        auth().oauth2(token).
//                // make call to API
//                        when().get("https://" + baseAddress + "/api/api/flight")
//                .then()
//                .body("size()", greaterThanOrEqualTo(0));
...
//        {flight:{delay:"bla", startTime:"11:00", }
        ...
        String json = given().auth().oauth2(token).get(url).getBody().print();
...
//      Prüft die Länge des Arrays mit Flügen >= 1
        given().auth().oauth2(token).get(url).then().body("size()", greaterThan(0));
//      Prüfen ob Delay !=null ist
        given().auth().oauth2(token).get(url).then().body("get(0).delay", notNullValue());
//      Prüft die Länge der FlightId(36)
        given().auth().oauth2(token).get(url).then().body("get(0).blueprint.flightnumber.length()", equalTo(36));
//      Prüft ob Flightnumber eine UUID ist
        given().auth().oauth2(token).get(url).then().body("get(0).blueprint.flightnumber", instanceOf(UUID.class));
//      Prüfen ob Startzeit !=null ist
        given().auth().oauth2(token).get(url).then().body("get(0).startTime", notNullValue());
//      Prüft die Länge der AircraftId(36)
        given().auth().oauth2(token).get(url).then().body("get(0).aircraft.length()", equalTo(36));
//      Prüft ob AircraftId eine UUID ist
        given().auth().oauth2(token).get(url).then().body("get(0).aircraft", instanceOf(UUID.class));
//      Prüfen ob Departure !=null ist
        given().auth().oauth2(token).get(url).then().body("get(0).blueprint.departure", notNullValue());
//      Prüfen ob Destination !=null ist
        given().auth().oauth2(token).get(url).then().body("get(0).blueprint.destination", notNullValue());
//      Prüfen ob Duration !=null ist
        given().auth().oauth2(token).get(url).then().body("get(0).blueprint.duration", notNullValue());
//      Prüfen ob Aircraftmodel !=null ist
        given().auth().oauth2(token).get(url).then().body("get(0).blueprint.aircraftmodel", notNullValue());
